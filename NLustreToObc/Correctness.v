...
Module Type CORRECTNESS
  ...
  Lemma stmt_eval_Control_fwd:
    forall prog menv env mems c s menv' env',
      ...
      Is_present_in mems menv env c /\ stmt_eval prog menv env s (menv', env')
      \/
      Is_absent_in mems menv env c /\ menv' = menv /\ env' = env.
  Proof.
  ...
  Lemma call_menv_inv:
    forall x prog m y ck f les mems menv env env' menv',
      stmt_eval prog menv env
                (Control mems ck (Call y f (hd default y) m les))
                (menv', env') ->
      mfind_mem x menv' = mfind_mem x menv.
  Proof.
    intros ** Hstep.
    apply stmt_eval_Control_fwd in Hstep;
  ...
  Proof. (* TODO: Tidy proof *)
    ...
    - apply stmt_eval_Control_fwd in Heval;
      ...
      + apply stmt_eval_translate_cexp_menv_inv in Heval.
        now rewrite Heval.
      + destruct Heval; subst; auto.
    - destruct o as [(r, ck_r)|].
      + inversion_clear Heval as [| | |????????? Hreset Hstep| |].
        eapply (call_menv_inv x) in Hstep.
        unfold reset_stmt in Hreset.
        apply stmt_eval_Control_fwd in Hreset.
        destruct Hreset as [[? Hreset]|[? Hreset]].
        * inversion_clear Hreset as [| | | |???????????? Heval|].
          destruct b; inversion_clear Heval; auto.
          subst; rewrite Hstep; auto.
        * destruct Hreset; subst; auto.
      + eapply call_menv_inv; eauto.
    - apply not_Is_defined_in_eq_EqFby in Hneq.
      unfold translate_eqn in Heval.
      apply stmt_eval_Control_fwd in Heval;
        destruct Heval as [[? Heval]|[? Heval]]; inv Heval; auto.
      now apply mfind_mem_gso.
  Qed.

  Lemma call_mobj_inv:
    forall x prog m y ck f les menv env mems menv' env',
      ~ In x y ->
      y <> [] ->
      stmt_eval prog menv env
                (Control mems ck (Call y f (hd default y) m les))
  ...
  Proof.
    intros ** Hnin Hnil Heval.
    apply stmt_eval_Control_fwd in Heval;
      destruct Heval as [[? Heval]|[? Heval]]; inv Heval; auto.
    apply mfind_inst_gso.
    intro; subst; apply Hnin.
    destruct y; try constructor; auto.
    now contradict Hnil.
  Qed.
  ...
  Lemma not_Is_defined_in_eq_stmt_eval_mobj_inv:
    forall prog x eq menv env mems menv' env',
      ~ Is_defined_in_eq x eq ->
      (forall ck f les r, eq <> EqApp [] ck f les r) ->
      stmt_eval prog menv env (translate_eqn mems eq) (menv', env') ->
      mfind_inst x menv' = mfind_inst x menv.
  Proof. (* TODO: Tidy proof *)
    ...
    - simpl in Heval.
      ...
      + destruct Heval; subst; auto.
    - simpl in Heval.
      assert (~ In x y) as Hnin by (intro; apply Hneq; now constructor).
      assert (y <> []) as Hnil by (intro; subst; eapply Hnemp; eauto).
      destruct o as [(r, ck_r)|].
      + inversion_clear Heval as [| | |????????? Hreset Hstep| |].
        eapply (call_mobj_inv x) in Hstep; eauto.
        unfold reset_stmt in Hreset.
        apply stmt_eval_Control_fwd in Hreset.
        destruct Hreset as [[? Hreset]|[? Hreset]].
        * inversion_clear Hreset as [| | | |???????????? Heval|].
  ...
  Proof.
    ...
    - apply stmt_eval_Control_fwd in Heval;
      ...
      + apply stmt_eval_translate_cexp_env_add in Heval.
        destruct Heval; subst.
        apply PM.gso; intro; subst; apply Hnd; constructor.
      + destruct Heval; subst; auto.
    - destruct o as [(r, ck_r)|].
      + inversion_clear Heval as [| | |????????? Hreset Hstep| |].
        eapply (call_env_inv x) in Hstep; eauto.
        unfold reset_stmt in Hreset.
        apply stmt_eval_Control_fwd in Hreset.
        destruct Hreset as [[? Hreset]|[? Hreset]].
        * inversion_clear Hreset as [| | | |???????????? Heval|].
  ...
  Lemma stmt_eval_translate_eqns_minst_inv:
    forall prog menv env mems eqs menv' env',
      stmt_eval prog menv env (translate_eqns mems eqs) (menv', env') ->
      forall x, ~ Is_defined_in_eqs x eqs ->
           (forall ck f les r, ~ In (EqApp [] ck f les r) eqs) ->
           mfind_inst x menv' = mfind_inst x menv.
  Proof.
    induction eqs as [ |eq].
    - inversion_clear 1; reflexivity.
    - intros ** Heval x Hnmem Hndef.
      ...
      assert (forall ck f les r, eq <> EqApp [] ck f les r)
...
for all [Is_free_exp x e]. *)
  ...
  Proof.
    ...
    induction e; intro; inversion 1; intros; subst; auto.
    - constructor; match goal with H: present _ = present _ |- _ => now inv H end.
    - split_env_assumption;
      unfold translate_lexp;
      match goal with y: ident |- _ => destruct (PS.mem y mems) eqn:Hm end;
  ...
  Proof.
    induction eqs as [|eq eqs IH];
      ...
      destruct Hmsem as [Hmeq Hmeqs]; auto.
    destruct eq; inversion Hidi; subst;
      try (exfalso; apply Hnvi0; now constructor).
    inversion_clear Hmeq as [| | |????????? Hsemls Hxs Hmfby];
      inversion_clear Hmfby as [????? Hmfind Hmls].
    specialize Hmls with n; specialize (Hxs n); simpl in Hxs.
    destruct (mfind_mem i (M n)); try contradiction.
    destruct (ls n);
      destruct Hmls as [? Hsv']; rewrite Hsv' in Hxs.
    - assert (present c = absent) by sem_det; discriminate.
    - assert (present v = present c) as E by sem_det; now inv E.
  Qed.
  ...
  auxilliary lemmas? *)
    ...
    Lemma is_step_correct:
      forall eqs,
        ...
        (forall x,
            PS.In x mems ->
            Is_defined_in_eqs x alleqs /\ ~Is_variable_in_eqs x alleqs) ->
        ...
        Forall (Memory_Corres_eq G (M n) menv) alleqs ->
    ...
    Proof.
      ...
      assert (exists menv' env',
               ...
               /\ (forall x, Is_variable_in_eqs x eqs ->
                       forall c, sem_var_instant (restr H n) x (present c)
                            <-> PM.find x env' = Some c)
               /\ Forall (Memory_Corres_eq G (M (S n)) menv') eqs) as IHeqs'.
      ...
      inversion Hsem as
          [??? y ??? Hvar Hce
          |??? y ???? les ?? Hsome Hmfind Hlaes Hvar Hmsem
          |??? y ???? les r ck_r ??? Hsome Hmfind Hlaes Hvar ? Hmsem
          |??? y ????? Hlae Hvar Hmfby
          ];
...
(* Focus 3. *)
      - (* Case EqDef: y = cae *)
          ...
          as Hce'.
        {
          ...
          - assert (~ Is_defined_in_eqs x eqs)
            ...
            match goal with
              ...
              eapply Is_memory_in_msem_var in H;
                try eassumption
            end.
        ...
        + (* xs n = present *)
          ...
          *{ intros x0 Hivi c.
             ...
             - destruct (ident_eq_dec x0 y) as [Hxy|Hnxy].
      ...
      - (* Case EqApp: y = f lae *)
          ...
          as Hlae'.
        {
          ...
          - assert (Hdecide_x: Is_variable_in_eqs x eqs \/ In x inputs).
            { eapply Is_well_sch_free_variable.
              - eauto.
              - apply FreeEqApp with Cbase; auto.
              - auto.
            }
            destruct Hdecide_x; try subst x.
            + apply IHeqs0; assumption.
          ...
          - assert (~ Is_defined_in_eqs x eqs).
            { eapply Is_well_sch_free_variable_in_mems.
              - eauto.
              - apply FreeEqApp with Cbase; auto.
              - auto.
            }
        ...
        assert (Forall (Memory_Corres_eq G (M n) menv) oeqs
                /\ Memory_Corres_eq G (M n) menv (EqApp xs ck f les None)
                /\ Forall (Memory_Corres_eq G (M n) menv) eqs)
          as (_ & Hmceq & Hmceqs)
        ...
        assert (Hsomexy: Some x = Some y) by (rewrite <-Hsome'; auto).
        ...
        inversion_clear Hmsem'
          as [? Hn ? ? ? ? ? Hck Hfind Hlsn Hxsn Hsamexs Hsameys Hout Hnsem].
        ...
        assert (~Is_defined_in_eqs y eqs) as Hniii.
        {
          ...
          assert (Is_defined_in_eq y (EqApp xs ck f les None))
            by now constructor.
        ...
        + (* y = present *)
          ...
          assert (exists cs, xss n = map present cs)
            as (outValues & Hxsc).
          {
            assert ((0 < length (idty n0.(n_in)))%nat) as ingt0'
                by (rewrite length_idty; apply n_ingt0).
          ...
          assert (exists menv' : heap,
                     ...
                     /\ Memory_Corres G f (M' (S n)) menv') as Hclass
              by (eapply Hnode; eauto).
          destruct Hclass as (omenv' & Hnstmt & Hnmc).

          simpl in *.
          exists (madd_obj y omenv' menv'), (adds xs outValues env').
          split;[|split].
          * apply stmt_eval_translate_eqns_cons.
            exists menv', env'.
            split; [exact Hstmt|].
            ...
            assert (Hhd: hd Ids.default xs = y).
            {
          ...
          *{ intros x Hivi c.
             ...
             - (* In x ys *)
               inversion_clear Hivi as [|?????? Hinx].
               clear - Hxsn Hinx Hvar.
               (* XXX: this should be an independant lemma *)
               assert ((0 < length (map fst n0.(n_out)))%nat) as Length
                   by (rewrite map_length; apply n_outgt0).
               revert Length.
               generalize dependent outValues.
               generalize n0.(n_out) as o.
               induction xs; simpl in *; try contradiction.
               intros o outValues Hvar Hxsn Length.
                 ...
                 as (outValue & outValues' & HoutValues).
               {
                 destruct outValues.
                 - inversion Hxsn as [E|].
                   rewrite <-E in Length; simpl in Length; omega.
                 - now eauto.
               }
               subst outValues.
               ...
               + (* Case: x = a *)
                ...
                + (* In x ys *)
                  ...
                  destruct Hinx as [|Hinx]; [ now contradiction | ]; simpl in *.
                  rewrite find_gsso; [| now congruence].

                  destruct outValues' eqn:HoutValues'; simpl in *.
                  * exfalso.
                    inversion_clear Hvar as [|????? Hf]; inv Hf; inv Hinx.
                  *{ assert (Forall2 (sem_var_instant (restr H n)) xs (map present outValues'))
                       by now inv Hvar.

                     destruct o; simpl in *; try omega.
                     destruct o eqn:Ho; simpl in *.
                     - inversion_clear Hxsn as [|????? Hxsn']; inv Hxsn'.
              ...
              - (* ~ In x ys *)
                assert (~ In x xs) by now (not_Is_variable x xs).
        ...
        + (* y = absent *)
          ...
          assert ((0 < length (idty n0.(n_in)))%nat) as ingt0'
              by (now rewrite length_idty; apply n_ingt0).

          assert (Habs: absent_list (ls n) ->
                        Forall (rhs_absent_instant (bk0 n) (restr Hn n)) n0.(n_eqs))
          ...
          assert (absent_list (xss n)) as Hout'
              by now apply Hout.
          ...
          * intros x Hivi c.
            ...
            assert (Hsv': Forall2 (sem_var_instant (restr H n)) xs (all_absent (xss n)))
            ...
            clear - Hsv' Hinx Hfind' Hsv.
            {
              ...
              remember (xss n) as xsn.
              clear Heqxsn.
              generalize dependent xsn.
              induction xs; intros xsn Hsv'; try inversion_clear Hinx.
              - subst a.
              ...
              - destruct xsn as [|v xsn]; [ now inv Hsv' | ].
                assert (Forall2 (sem_var_instant (restr H n)) xs (all_absent xsn))
                  by now inv Hsv'.
                eapply IHxs; eauto.
            }
      ...
      - (* Case EqReset *)
          ...
          as Hlae'.
        {
          ...
          - assert (Hdecide_x: Is_variable_in_eqs x eqs \/ In x inputs).
            { eapply Is_well_sch_free_variable.
              - eauto.
              - apply FreeEqApp with ck_r; auto.
              - auto.
            }
            destruct Hdecide_x; try subst x.
            + apply IHeqs0; assumption.
        ...
        (* memory correspondence before execution *)
        assert (Forall (Memory_Corres_eq G (M n) menv) oeqs
                /\ Memory_Corres_eq G (M n) menv (EqApp xs ck f les (Some (r, ck_r)))
                /\ Forall (Memory_Corres_eq G (M n) menv) eqs)
          as (_ & Hmceq & Hmceqs)
            by now (rewrite <- Forall_cons2; apply Forall_app; rewrite <- Hall).
        clear Hmc.
        ...
        edestruct Hmc0 as [omenv [Hfindo Hmc0']]; eauto.
        (* dataflow semantics *)
        inversion_clear Hmsem as [????? Hsem_node].
        set (rk := reset_of ys) in *.
        destruct (Hsem_node (count rk n)) as (M'' & Hmsem' & Mask).
        assert (Hmsem'':=Hmsem').
        inversion_clear Hmsem''
          as [? Hn ? ? ? ? ? Hck Hfind Hlsn Hxsn Hsamexs Hsameys Hout Hnsem].
        ...
        + (* y = present *)
          ...
          assert (Hlen: (0 < length (ls n))%nat).
          { unfold sem_vars, lift in Hlsn; specialize (Hlsn n).
            rewrite mask_transparent in Hlsn; auto.
            apply Forall2_length in Hlsn; rewrite <-Hlsn, map_length.
            apply n_ingt0.
          }

          assert (~ absent_list (mask (all_absent (ls 0%nat)) (count rk n) rk ls n)).
          { intro Habs.
            unfold absent_list, mask in Habs.
            rewrite <-EqNat.beq_nat_refl in Habs.
            destruct (ls n); try now inv Hlen.
            destruct cs; try discriminate.
            rewrite Hlsp in Habs.
            inv Habs; destruct cs; discriminate.
          }

          assert (exists cs, mask (all_absent (ls 0%nat)) (count rk n) rk ls n = map present cs)
            as (inValues & Hlsc).
          { assert (present_list (mask (all_absent (ls 0%nat)) (count rk n) rk ls n))
              by now apply present_not_absent_list; eauto.

            now apply present_list_spec.
          }

          assert (exists cs, mask (all_absent (xss 0%nat)) (count rk n) rk xss n = map present cs)
            as (outValues & Hxsc).
          { assert (present_list (mask (all_absent (xss 0%nat)) (count rk n) rk xss n))
                   by (apply present_not_absent_list; eauto; now rewrite <-Hout).

            now apply present_list_spec.
          }
          rewrite Hxsc, Hlsc in *.

          assert (exists menv',
                     stmt_call_eval prog omenv f step inValues menv' outValues
                     /\ Memory_Corres G f (M'' (S n)) menv') as Hclass
              by (eapply Hnode; eauto; rewrite Mask; auto).
        ...
        + (* y = absent *)
          ...
          assert (Habs: absent_list (mask (all_absent (ls 0%nat)) (count rk n) rk ls n) ->
                        Forall (rhs_absent_instant (bk0 n) (restr Hn n)) n0.(n_eqs))
            by (eapply subrate_property_eqns; eauto;
                eapply sem_vars_gt0 with (1:=ingt0');
                rewrite map_fst_idty; apply Hlsn).
          ...
          { apply absent_list_mask.
            - rewrite H2; apply all_absent_spec; auto.
            - apply all_absent_spec.
          }

          assert (absent_list (mask (all_absent (xss 0%nat)) (count rk n) rk xss n)) as Hout'
              by now apply Hout.
          ...
          * intros x Hivi c.
            ...
            assert (Hsv': Forall2 (sem_var_instant (restr H n)) xs (all_absent (xss n))).
            { apply absent_list_spec in Hout'.
              rewrite all_absent_mask in Hout'; auto.
              rewrite <-Hout', mask_transparent; auto.
            }

            assert (Hinx: In x xs)
              by now inversion_clear Hivi'.

            clear - Hsv' xss Hinx Hfind' Hsv.
            {
              (* XXX: Refactor as standalone Lemma *)
              remember (xss n) as xsn.
              clear Heqxsn.
      ...
      - (* Case EqFby: y = v0 fby lae *)
        inversion_clear Hmfby as [?????? Hy];
          specialize (Hy n);
          destruct (mfind_mem y (M n)) eqn: Hms; try contradiction.
        specialize (Hlae n).
        ...
        { intros.
          ...
          - assert (~ Is_defined_in_eqs x eqs)
            ...
            specialize (Hinmems _ Hmems); destruct Hinmems.
            erewrite stmt_eval_translate_eqns_menv_inv; try eassumption.
            match goal with H: sem_var_instant _ _ _ |- _ =>
                            eapply Is_memory_in_msem_var in H; try eassumption
            end.
            assert (Is_defined_in_eqs x alleqs) by intuition.
            assert (~ Is_variable_in_eqs x alleqs) by intuition.
            erewrite Is_memory_in_Memory_Corres_eqs; try eauto.
        ...
        inversion Hlae; subst ck0 le;
          match goal with
          | H: present _ = ls n |- _ => rewrite <- H in *
          | H: absent = ls n |- _ => rewrite <- H in *
          end;
        destruct Hvar as [Hvar], Hy as (Hmfind & Hxs).
        + (* y = present *)
          rename c into v'.
    ...
    Proof.
      ...
      - intros Hwd f n xs ys M menv inputs output Hmc Hmsem Hxs Hys.
        ...
        + (* Case: f = nodeName *)
          ...
          clear Hwd'.
          inversion_clear Hmsem as [??????? Hck Heqs Hin Hout Hrabs].
          subst eqs nodeName.
          simpl in Heqs; rewrite Hfeq in Heqs; simpl in Heqs.
          injection Heqs. intro Hnode.
          subst n0.
          (* rewrite Hnode in *. clear Heqs. simpl in *. *)
          ...
          assert (exists (menv' : heap) (env' : stack),
                     stmt_eval (translate G) menv env
                               (translate_eqns (DF.Mem.memories node.(n_eqs)) node.(n_eqs))
                       (menv', env') /\
                     (forall x : ident,
                         ...
                         forall c : val,
                           sem_var_instant (restr H n) x (present c) <->
                           PM.find x env' = Some c) /\
                     Forall (Memory_Corres_eq G (M (S n)) menv') node.(n_eqs))
            as His_step_correct.
          {
            assert (bk n = true).
            {
            ...
            assert (forall x,
                       PS.In x (DF.Mem.memories (n_eqs node)) ->
                       ...
                       PM.find input env = Some c)).
            {
              ...
              assert (NoDupMembers node.(n_in))
                by (eapply NoDupMembers_app_l; eapply n_nodup; eauto).
            ...
            assert (Forall (Memory_Corres_eq G (M n) menv) (n_eqs node)).
            {
              inversion_clear Hmc as [???? Hf Hmeqs].
              simpl in Hf.
              rewrite ident_eqb_refl in Hf.
              injection Hf; intros Heq0; subst n0.
  ...
  Definition equiv_reset G prog f :=
    forall xs ys M,
      ...
      exists menv',
        ...
        /\ Memory_Corres G f (M 0%nat) menv'.
      ...
      forall bk eqs,
        ...
        exists menv' env',
          stmt_eval (translate G) menv sempty (translate_reset_eqns eqs)
                    (menv', env')
          /\ Forall (Memory_Corres_eq G (M 0) menv') eqs.
    Proof.
      intros * Hmsem Hwsch Hreset.
      induction eqs as [|eq eqs IH]; [eauto|].
      intro menv.
      destruct eq as [i ck e | is ck f e [(r, ck_r)|]| i ck v e];
      ...
      - (* EqApp *)
        ...
        assert (exists omenv',
                   ...
                   /\ Memory_Corres G f (Mo 0%nat) omenv')
        ...
        + rewrite stmt_eval_translate_reset_eqn_shift.
          ...
          subst omenv.
          econstructor; eauto.
        + assert (Memory_Corres_eq G (M 0) (madd_obj i omenv' menv') (EqApp is ck f e None)).
          {
            econstructor; eauto.
            intros M' Hmfind'.
          ...
          assert (Forall (Memory_Corres_eq G (M 0) (madd_obj i omenv' menv')) eqs).
          {
            assert (Is_defined_in_eq i (EqApp is ck f e None)).
            {
      ...
      - (* EqFby *)
        ...
        + unfold translate_reset_eqns; simpl;
          ...
          Hint Constructors stmt_eval.
          eauto.
        + inversion_clear Hsem as [| | |? ? ? ? ? ? ? ? ? ? ? Hmfby];
            inversion_clear Hmfby as [????? Hmfind Hms].
          (* rewrite <- Hms. *)
          constructor; [|apply Memory_Corres_eqs_add_mem; assumption].
          constructor. intros v' Hmfind'.
  ...
  Proof.
    ...
    - intros f Hwdef xs ys M Hmsem menv.
      ...
      + assert (nodeName = f) as Hfeq
            by (apply Pos.eqb_eq; assumption).
        inversion_clear Hmsem as [? ? ? ? ? ? ? Hbk Hfind Hin Hout Hrhs].
        ...
        assert (msem_equations G bk H M n.(n_eqs)).
        {
          inversion_clear Hwdef. subst n.
          now eapply Forall_msem_equation_global_tl; eauto.
        }

        set (inArgs_fst := map fst n.(n_in)).
        assert (Is_well_sch (DF.Mem.memories n.(n_eqs)) inArgs_fst n.(n_eqs))
          by now inversion Hwdef; subst inArgs_fst n.
        ...
        assert (exists menv' env',
                   stmt_eval (translate G) menv sempty
                             ...
                             (menv', env')
                   /\ Forall (Memory_Corres_eq G (M 0%nat) menv') n.(n_eqs))
        ...
        *{ econstructor.
           ...
           - apply Memory_Corres_eqs_node_tl; try assumption.
             inversion Hwdef; now rewrite <-Hfind.
         }
  ...
  Section EventLoop.
    ...
    Proof.
      ...
      assert (exists co0, yss 0 = map present co0)%nat as [co0 Hco0].
      {
        inversion_clear Hmsem as
            [??????? Hbk Hfind
                Hsem_in Hsem_out Hsamexs Hsameys Habs Hsem_eqns].
        ...
        assert (Hlen: 0 < length ci0).
        {
          ...
          assert (length (map fst n0.(n_in)) = length (xss 0)) as <-
      ...
      - (* Case: n ~ S n *)
        ...
        assert (exists coSn, yss (S n) = map present coSn) as [coSn Hys].
        {
          ...
          assert (Hlen: 0 < length ciSn).
          {
            ...
            assert (length (map fst n0.(n_in)) = length (xss (S n))) as <-
                by now eapply Forall2_length;
    ...
    Lemma dostep'_init:
      forall M,
        ...
        exists menv0 c_main P',
          ...
          /\ Memory_Corres G main (M 0) menv0
    ...
    set (R := fun n menv => Memory_Corres G main (M n) menv).
