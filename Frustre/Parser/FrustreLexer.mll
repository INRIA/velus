...
let lexicon : (string, FrustreAst.astloc -> token) Hashtbl.t = Hashtbl.create 17
...
let currentLoc =
  ...
  fun lb ->
    ...
    FrustreAst.({ ast_lnum  = p.Lexing.pos_lnum;
                 ast_fname = p.Lexing.pos_fname;
                 ast_bol   = p.Lexing.pos_bol;
                 ast_cnum  = p.Lexing.pos_cnum;
                 ast_ident = getident ();})

let string_of_loc { FrustreAst.ast_fname = fname;
                    FrustreAst.ast_lnum  = lnum;
                    FrustreAst.ast_bol   = bol;
                    FrustreAst.ast_cnum  = cnum } =
  Printf.sprintf "%s:%d:%d" fname lnum (cnum - bol + 1)

let lexing_loc { FrustreAst.ast_lnum  = lnum;
                 FrustreAst.ast_fname = fname;
                 FrustreAst.ast_bol   = bol;
                 FrustreAst.ast_cnum  = cnum } =
...
rule initial = parse
  ...
  | "//"                          { singleline_comment lexbuf; initial lexbuf }
  | "--"                          { singleline_comment lexbuf; initial lexbuf }
  | integer_constant as s         { tok CONSTANT't (FrustreAst.CONST_INT s,
                                                    currentLoc lexbuf) }
  | decimal_floating_constant     { tok CONSTANT't (FrustreAst.CONST_FLOAT
                                      {FrustreAst.isHex_FI = false;
                                       FrustreAst.integer_FI = intpart;
                                       FrustreAst.fraction_FI = frac;
                                       FrustreAst.exponent_FI = expo;
                                       FrustreAst.suffix_FI =
                                         match suffix with
                                         | None -> None
                                         | Some c -> Some (String.make 1 c) },
                                      currentLoc lexbuf) }
  | hexadecimal_floating_constant { tok CONSTANT't (FrustreAst.CONST_FLOAT
                                      {FrustreAst.isHex_FI = true;
                                       FrustreAst.integer_FI = intpart;
                                       FrustreAst.fraction_FI = frac;
                                       FrustreAst.exponent_FI = Some expo;
                                       FrustreAst.suffix_FI =
                                         match suffix with
                                           | None -> None
                                           | Some c -> Some (String.make 1 c) },
  ...
  | "'"                           { let l = char_literal lexbuf.lex_start_p []
                                              lexbuf in
                                    tok CONSTANT't
                                          (FrustreAst.CONST_CHAR(false, l),
                                                    currentLoc lexbuf) }
  | "L'"                          { let l = char_literal lexbuf.lex_start_p []
                                              lexbuf in
                                    tok CONSTANT't
                                          (FrustreAst.CONST_CHAR(true, l),
                                                    currentLoc lexbuf) }
