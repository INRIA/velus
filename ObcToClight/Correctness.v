...
Section PRESERVATION.
  ...
  Section MatchStatesAssign.
    ...
    Section OutField.
      Variables (m: Mem.mem) (ve: stack) (outco: composite) (outb: block) (P: massert)
                (le: temp_env) (x: ident) (ty: type).
      Hypothesis Hrep: m |= match_out owner caller ve le outb outco ** varsrep caller ve le(* blockrep gcenv ve outco.(co_members) outb *) ** P.
    ...
    Proof.
      ...
      - inv Length1; inv Length2; inv Nodup; inv Nodup'.    
        ...
        + assert (m_out caller <> []) as Hout
          ...
          rewrite sep_swap, match_out_notnil in Hrep; eauto.
          destruct Hrep as (Hrep & ? & ?).          
          ...
          * rewrite sep_swap3, sep_swap, match_out_notnil, sep_swap; auto; split; auto.            
            simpl in Hm2.
        ...
        forall owner c caller callee prog' prog'' me ve e1 le1 m1 o cf ptr_f sb
          ...
          exists m2 ws xs,
            ...
            /\ m2 |= staterep gcenv prog owner.(c_name) (madd_obj o me2 me) sb (Int.unsigned sofs)
                   ** blockrep gcenv ve outco.(co_members) outb
                   ** blockrep gcenv (adds (map fst callee.(m_out)) rvs sempty) outco_callee.(co_members) outb_callee
  ...
  Section Init.
    ...
    Lemma exec_write:
      forall ys ve le m,
        ...
        m |= blockrep gcenv (adds (map fst m_step.(m_out)) ys ve) (co_members step_co) step_b 
            ** P ->
    ...
    the [while] loop. *)
      ...
      Section Dostep'_coind.
        ...
        Hypothesis StepCase: forall n me,
            ...
            exists me',
              ...
              /\ me' |= blockrep gcenv (adds (map fst (m_out m_step)) (map sem_const (outs n)) sempty)
                               (co_members step_co) step_b 
                      ** P 
              /\ R (S n) me'.
      ...
      Proof.
        ...
        2: now exists me0; repeat (split; auto).
                 ...
                 clear - Hwt_ins Hwt_prog (* Hwt_mem *)WT_mem m_step Find Findstep.
