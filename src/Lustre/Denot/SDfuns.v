...
Section SStream_functions.
  ...
  (* Definition sfby1f : (A -O-> DS (sampl A * sampl A) -C-> DS (sampl A)) -C-> *)
  (*                     (A -O-> DS (sampl A * sampl A) -C-> DS (sampl A)). *)
  (*   apply ford_fcont_shift. *)
  (*   intro v. *)
  (*   apply curry. *)
  (*   apply (fcont_comp2 (DSCASE (sampl A * sampl A) (sampl A))). *)
  (*   2: exact (SND _ _). *)
  (*   apply ford_fcont_shift. *)
  (*   intros [vx vy]. *)
  (*   apply curry. *)
  (*   match goal with *)
  (*   | |- _ (_ (Dprod ?pl ?pr) _) => *)
  (*       pose (fby1 := fcont_ford_shift _ _ _ (FST _ _ @_ (FST pl pr))) *)
  (*   end. *)
  (*   exact (let (r,v) := match vx, vy with *)
  (*                       | err e, _ => (err e, v) *)
  (*                       | _, err e => (err e, v) *)
  (*                       | abs, abs => (abs, v) *)
  (*                       | pres a, pres b => (pres v, b) *)
  (*                       | _, _ => (err error_Cl, v) *)
  (*                       end *)
  (*          in *)
  (*          CONS r @_ (AP _ _ @2_ fby1 v) (SND _ _) *)
  (*         ). *)
  (* Defined. *)

  (* Lemma sfby1f_eq : forall f v x y xys, *)
  (*     sfby1f f v (cons (x, y) xys) *)
  (*     == *)
  (*       match x, y with *)
  (*       | err e, _ => cons (err e) (f v xys) *)
  (*       | _, err e => cons (err e) (f v xys) *)
  (*       | abs, abs => cons abs (f v xys) *)
  (*       | pres a, pres b => cons (pres v) (f b xys) *)
  (*       | _, _ => cons (err error_Cl) (f v xys) *)
  (*       end. *)
  (* Proof. *)
  (*   intros. *)
  (*   unfold sfby1f. *)
  ...
  (* Definition sfby1 : A -O-> DS (sampl A) -C-> DS (sampl A) -C-> DS (sampl A) := *)
  (*   fun v => curry (FIXP _ sfby1f v @_ (ZIP pair @2_ FST _ _) (SND _ _ )). *)

  (* Lemma sfby1_eq : forall v x xs y ys, *)
  (*     sfby1 v (cons x xs) (cons y ys) *)
  (*     == match x, y with *)
  (*       | err e, _ => cons (err e) (sfby1 v xs ys) *)
  (*       | _, err e => cons (err e) (sfby1 v xs ys) *)
  (*       | abs, abs => cons abs (sfby1 v xs ys) *)
  (*       | pres a, pres b => cons (pres v) (sfby1 b xs ys) *)
  (*       | _, _ => cons (err error_Cl) (sfby1 v xs ys) *)
  (*       end. *)
  (* Proof. *)
  (*   intros. *)
  (*   unfold sfby1. *)
  (*   assert (Heq:=FIXP_eq sfby1f). *)
  (*   rewrite (ford_eq_elim Heq v) at 1. *)
  (*   autorewrite with localdb using (simpl (snd _); simpl (fst _)). *)
  (*   now rewrite sfby1f_eq. *)
  (* Qed. *)

  (* Definition sfbyf : (DS (sampl A * sampl A) -C-> DS (sampl A)) -C-> *)
  (*                    (DS (sampl A * sampl A) -C-> DS (sampl A)). *)
  (*   apply curry. *)
  (*   apply (fcont_comp2 (DSCASE (sampl A * sampl A) (sampl A))). *)
  (*   2: exact (SND _ _). *)
  (*   apply ford_fcont_shift. *)
  ...
  (*   match goal with *)
  (*   | |- _ (_ (Dprod ?pl ?pr) _) => *)
  (*       pose (fby := (FST _ _ @_ (FST pl pr))) *)
  (*   end. *)
  (*   exact *)
  (*     (match vx, vy with *)
  (*      | pres a, pres b => CONS (pres a) @_ (FIXP _ sfby1f b) @_ (SND _ _) *)
  (*      | err _ as r, _ *)
  (*      | _, err _ as r *)
  (*      | abs, abs as r =>  CONS r    @_ ((AP _ _ @2_ fby) (SND _ _)) *)
  (*      | _, _ => CONS (err error_Cl) @_ ((AP _ _ @2_ fby) (SND _ _)) *)
  (*      end) . *)
  (* Defined. *)

  (* Definition sfbyf_eq : forall f x y xys, *)
  (*     sfbyf f (cons (x, y) xys) *)
  (*     == match x, y with *)
  (*        | pres x, pres y => cons (pres x) (FIXP _ sfby1f y xys) *)
  (*        | err _ as r, _ *)
  (*        | _, err _ as r *)
  (*        | abs, abs as r => cons r (f xys) *)
  (*        | _, _ => cons (err error_Cl) (f xys) *)
  (*        end. *)
  (* Proof. *)
  (*   intros. *)
  (*   unfold sfbyf. *)
  ...
  (* Definition sfby : DS (sampl A) -C-> DS (sampl A) -C-> DS (sampl A) := *)
  (*   curry (FIXP _ sfbyf @_ (ZIP pair @2_ FST _ _) (SND _ _ )). *)

  (* Lemma sfby_eq : forall x y xs ys, *)
  (*     sfby (cons x xs) (cons y ys) *)
  (*     == match x, y with *)
  (*        | pres x, pres y => cons (pres x) (sfby1 y xs ys) *)
  (*        | err _ as r, _ *)
  (*        | _, err _ as r *)
  (*        | abs, abs as r => cons r (sfby xs ys) *)
  (*        | _, _ => cons (err error_Cl) (sfby xs ys) *)
  (*        end. *)
  (* Proof. *)
  (*   intros. *)
  (*   unfold sfby. *)
  (*   autorewrite with localdb using (simpl (snd _); simpl (fst _)). *)
  (*   assert (Heq:=FIXP_eq sfbyf). *)
  (*   rewrite (fcont_eq_elim Heq). *)
  (*   now rewrite sfbyf_eq. *)
  (* Qed. *)

  (* (* TODO: doc *) *)
  (* Definition fby1P : DS (sampl A) -C-> DS (sampl A) -C-> DS (sampl A). *)
  (*   apply curry. *)
  ...
  (*   apply curry. *)
  (*   refine match y with *)
  (*   | pres v => (sfby1 v @2_ (FST _ _ @_ FST _ _)) (SND _ _) *)
  (*   | err _ => CTE _ _ (DS_const y) *)
  (*   | abs => CTE _ _ (DS_const (err error_Cl)) *)
  (*   end. *)
  (* Defined. *)

  (* Lemma fby1P_eq : forall xs y ys, *)
  (*     fby1P xs (cons y ys) *)
  (*     == match y with *)
  (*        | pres v => sfby1 v xs ys *)
  (*        | err _ => DS_const y *)
  (*        | abs => DS_const (err error_Cl) *)
  (*        end. *)
  (* Proof. *)
  (*   intros. *)
  (*   unfold fby1P. *)
