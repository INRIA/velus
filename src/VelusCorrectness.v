...
Section Bisim.
  ...
  (** The trace generated by [mk_events] is characterized by [match_trace] *)
  Lemma mk_event_spec:
    forall (f: eventval -> ident * type -> event) vs xts,
      wt_vals vs xts ->
      match_trace f vs xts (mk_events (fun v => f (eventval_of_val v)) vs xts).
  Proof.
    intros * Hwt. generalize dependent xts.
    induction vs as [|v vs IHvs];
      intros xts Hwt; destruct xts as [|[x t] xts];
        inv Hwt; try constructor.
    rewrite Traces.mk_events_cons.
  ...
  CoInductive bisim_io_from: nat -> traceinf -> Prop :=
    Step:
      forall node n t t' T,
        find_node main G = Some node ->
        ...
        bisim_io_from (S n) T ->
        bisim_io_from n (t *** t' *** T).

  Definition bisim_io := bisim_io_from 0.
...
Program Lemma find_node_trace_step:
  forall G f node ins outs m
    ...
    bisim_io G f ins outs
             (traceinf_of_traceinf'
                (trace_step m (tr_Streams ins) (tr_Streams outs) _ _ _ _ 0)).
Proof.
...
Proof.
  ...
  - (* assert some equalities between classes and methods *)
    ...
    rewrite <-Obc.Fus.fuse_method_in, <-add_defaults_method_m_in, Eq_step in Step_in_spec, Hwt_in;
      rewrite <-Obc.Fus.fuse_method_out, <-add_defaults_method_m_out, Eq_step in Step_out_spec, Hwt_out.
    ...
    + assert (Forall Obc.Fus.ClassFusible tr_sch_tr_G)
      ...
      eapply correctness with (TRANSL := COMP') (me0 := me0)
                              ...
                              (Step_in_spec := Step_in_spec) (Step_out_spec := Step_out_spec); eauto.
    ...
    + eapply find_node_trace_step; eauto; rewrite <-Eq_step; auto.
  ...
  - (* assert some equalities between classes and methods *)
    ...
    rewrite <-add_defaults_method_m_in, Eq_step in Step_in_spec, Hwt_in;
      rewrite <-add_defaults_method_m_out, Eq_step in Step_out_spec, Hwt_out.
