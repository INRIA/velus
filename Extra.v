...
Definition clight1_to_asm_passes :=
...
Definition match_prog: Clight.program -> Asm.program -> Prop :=
  pass_match (compose_passes clight1_to_asm_passes).


(** The [transf_clight_program] function, when successful, produces
...
Ltac DestructM :=
  ...
  assert (F: forward_simulation (Clight.semantics1 p) (Asm.semantics p20)).
  {
...
Theorem behavior_preservation:
  forall p tp,
  transf_clight_program p = OK tp ->
  forall B, (forall t, B <> Goes_wrong t) ->
  program_behaves (Clight.semantics1 p) B -> program_behaves (Asm.semantics tp) B.
Proof.
  intros.
  assert (M: match_prog p tp) by (apply transf_clight_program_match; auto).
  exploit clight_semantic_preservation; eauto. intros [FWD BACK].
  exploit forward_simulation_behavior_improves; eauto. intros (beh & B' & I).
  inv I. auto. destruct H2 as (t & ? & ?); specialize (H0 t); contradiction.
Qed.
...
Theorem reacts_trace_preservation:
  ...
  transf_clight_program p = OK tp ->
  forall T,
  program_behaves (Clight.semantics1 p) (Reacts T) -> program_behaves (Asm.semantics tp) (Reacts T).
Proof.
  intros.
  eapply behavior_preservation; eauto.
  intros ** ?. discriminate.
Qed.
