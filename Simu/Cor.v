...
Section PRESERVATION.
  ...
  with state_context: state -> Clight.cont -> Clight.env -> Clight.temp_env -> Memory.Mem.mem -> Prop :=
       ...
       | ctxt_skip_comp: forall me ve s k e le m,
           match_states
             (me, ve, Skip)
             (Clight.State main_fun
  ...
  Proof.
    ...
    - repeat app_stmt_eval_det.
      edestruct IHstmt_eval1 as [S2' [t1 [Step1 Match2]]]; eauto.
      assert (S2' = Clight.State main_fun (translate_stmt main_node Skip)
                                 (Clight.Kseq (translate_stmt main_node s2) k) e le m'). admit.
      edestruct IHstmt_eval2 as [S3' [t2 [Step2 Match3]]]; eauto.
      assert (S3' = Clight.State main_fun (translate_stmt main_node Skip) k e le m''). admit.
      exists S3', Events.E0; split; subst S2' S3'.      
      ...
      + inv Match3; constructor; (tauto || auto). 

    (* Comp Skip s : "skip; s" *)
    - repeat app_stmt_eval_det.
      edestruct IHstmt_eval as [S2' [t1 [Step1 Match2]]]; eauto.
      exists S2', Events.E0; split; auto.
      assert (S2' = Clight.State main_fun (translate_stmt main_node s) k e le m). admit. subst S2'.
      eapply Smallstep.plus_left' with
